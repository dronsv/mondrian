...
| `mondrian.native.nonEmptyCrossJoin.judgeCells.enable` | `true` | **не трогать** | Внутренний диагностический аварийный выключатель для канареечных прогонов, не поддерживаемая настройка. При `true` `NonEmptyCrossJoin` проверяет каждую пару по её собственной координате. `false` возвращает прежнее, заведомо некорректное поведение: в результат попадают пустые пары. Тесты рассчитаны на `true`: с `false` 14 тестов падают намеренно |
