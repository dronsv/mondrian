...
public class MondrianProperties extends MondrianPropertiesBase {
     ...
     * <code>DefaultMember</code> during evaluation unless those hierarchies are
     * explicitly referenced in the formula.</p>
     *
     * <p>This is intended as an opt-in convenience for flat-hierarchy share
     * measures where peer hierarchies would otherwise leak axis or slicer context
     * into the denominator.</p>
     */
    public transient final BooleanProperty CalcShareMeasureAutoResetPeerHierarchies =
        new BooleanProperty(
            this, "mondrian.calc.shareMeasure.autoResetPeerHierarchies", false);
    ...
    /**
     * <p>
     *                 If true, all session caches are cleared when an OutOfMemory exception occurs.
     *                 Default: false.
     *             </p>
     */
    public transient final BooleanProperty ClearCachesOnOutOfMemory =
        new BooleanProperty(
            this, "mondrian.server.ClearCachesOnOutOfMemory", false);

    /**
     * <p>Comma-separated allowlist of setting names permitted in generated
     * ClickHouse <code>SETTINGS</code> clauses.</p>
     *
     * <p>Default allowlist includes:
     * <code>max_threads,max_execution_time,max_memory_usage,
     * max_bytes_before_external_group_by,max_bytes_before_external_sort,
     * max_rows_to_read,max_bytes_to_read,max_result_rows,max_result_bytes,readonly</code>.</p>
     */
    public transient final StringProperty ClickHouseSqlSettingsAllowlist =
        new StringProperty(
            this, "mondrian.clickhouse.sqlSettings.allowlist", "");

    /**
     * <p>Optional per-catalog override map for ClickHouse statement
     * settings.</p>
     *
     * <p>Format:
     * <code>catalogA=name=value,name2=value2;catalogB=name=value</code>.</p>
     *
     * <p>Overrides are applied after global settings.</p>
     */
    public transient final StringProperty ClickHouseSqlSettingsByCatalog =
        new StringProperty(
            this, "mondrian.clickhouse.sqlSettings.byCatalog", "");

    /**
     * <p>Boolean property that enables ClickHouse query-level
     * <code>SETTINGS</code> passthrough in generated SQL.</p>
     *
     * <p>When true, Mondrian appends allowlisted settings configured via
     * <code>ClickHouseSqlSettingsGlobal</code> and
     * <code>ClickHouseSqlSettingsByCatalog</code>.</p>
     *
     * <p>Default is false.</p>
     */
    public transient final BooleanProperty ClickHouseSqlSettingsEnabled =
        new BooleanProperty(
            this, "mondrian.clickhouse.sqlSettings.enabled", false);

    /**
     * <p>Comma-separated list of global ClickHouse statement settings.</p>
     *
     * <p>Format: <code>name=value,name2=value2</code>.</p>
     *
     * <p>Only settings present in
     * <code>ClickHouseSqlSettingsAllowlist</code> and with safe scalar values
     * are emitted.</p>
     */
    public transient final StringProperty ClickHouseSqlSettingsGlobal =
        new StringProperty(
            this, "mondrian.clickhouse.sqlSettings.global", "");
    ...
    /**
     * <p>Controls dependency-aware pruning of member-list native crossjoin arguments.</p>
     *
     * <p>Recognized values:</p>
     * <ul>
     * <li><code>OFF</code> - disable dependency pruning</li>
     * <li><code>STRICT</code> - only apply validated explicit
     * <code>drilldown.dependsOn</code> rules</li>
     * <li><code>RELAXED</code> - apply validated explicit rules and same-hierarchy
     * ancestor fallback (default)</li>
     * </ul>
     */
    public transient final StringProperty CrossJoinDependencyPruningPolicy =
        new StringProperty(
    ...
    /**
     * <p>When enabled, native NON EMPTY CrossJoin results are stable-reordered into
     * tuple-column order when the projected levels are linked by validated
     * <code>drilldown.dependsOnChain</code> rules.</p>
     *
     * <p>This is intended as a safe Excel-oriented quick fix for flat hierarchy
     * chains where child members should remain visually clustered under preceding
     * parent blocks without changing the SQL planner.</p>
     */
    public transient final BooleanProperty CrossJoinOrderByDependsOnChain =
        new BooleanProperty(
            this, "mondrian.native.crossjoin.orderByDependsOnChain", false);
    ...
    /**
     * <p>Boolean property enabling wider aggregate-table matching for
     * distinct-count merge mode in constrained (slicer/WHERE) and all-level
     * (no GROUP BY) queries.</p>
     *
     * <p>When true and <code>DistinctCountMergeFunction</code> is configured,
     * Mondrian allows distinct merge batches to roll up across the aggregate
     * table grain (instead of restricting matching to
     * <code>rollableLevelBitKey</code> only). This helps constrained
     * distinct-count queries and warmup all-level totals use aggregate tables
     * instead of fact fallback.</p>
     *
     * <p>Default is false for safety. Enable only when distinct-count measures
     * are stored as mergeable aggregate states compatible with the configured
     * merge function.</p>
     */
    public transient final BooleanProperty DistinctCountMergeAllowConstrainedRollup =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.DistinctCountMergeAllowConstrainedRollup", false);

    /**
     * <p>String property that specifies a database-specific merge function
     * for distinct-count measures stored as aggregate states in aggregate
     * tables.</p>
     *
     * <p>When set (e.g. to "uniqCombinedMerge" for ClickHouse), Mondrian
     * will allow aggregate tables with ignored columns to be used for
     * distinct-count measures and will generate the specified function
     * instead of SUM for rolling up pre-aggregated distinct-count states.</p>
     *
     * <p>When empty or not set, original behavior is preserved: aggregate
     * tables with ignored columns are skipped for distinct-count measures.</p>
     */
    public transient final StringProperty DistinctCountMergeFunction =
        new StringProperty(
    ...
    public transient final StringProperty DistinctCountMergeFunctionMap =
        new StringProperty(
            this, "mondrian.rolap.aggregates.DistinctCountMergeFunctionMap", "");

    /**
     * <p>String property controlling how
     * <code>DistinctCountMergeFunction</code> is enabled.</p>
     *
     * <p>Supported values:</p>
     * <ul>
     * <li><code>off</code> - always disable merge-state routing;</li>
     * <li><code>auto</code> - enable only when the SQL dialect declares support
     * for the configured merge function;</li>
     * <li><code>on</code> - force-enable merge-state routing regardless of dialect
     * capability checks.</li>
     * </ul>
     *
     * <p>Default is <code>auto</code>. Invalid values fall back to
     * <code>auto</code>.</p>
     */
    public transient final StringProperty DistinctCountMergeMode =
        new StringProperty(
            this, "mondrian.rolap.aggregates.DistinctCountMergeMode", "auto");

    /**
     * <p>If enabled, <code>SqlTupleReader</code> will only choose aggregate
     * tables that can enumerate members without reverse joining aggregate
     * level values back into dimension tables.</p>
     *
     * <p>When enabled, Mondrian rejects aggregate candidates that would require
     * string-value equality joins such as <code>dim.level = agg.level</code>
     * for tuple enumeration, and falls back to another feasible aggregate or
     * the fact table.</p>
     */
    public transient final BooleanProperty EnableAggTupleEnumerationFeasibility =
        new BooleanProperty(
            this, "mondrian.rolap.tupleReader.aggFeasibleEnumeration.enable", true);
    ...
    /**
     * <p>
     *                 The server will close a session if it is not in use for the specified timeout.
     *                 Default value is 3600 seconds.
     *             </p>
     */
    public transient final IntegerProperty IdleOrphanSessionTimeout =
        new IntegerProperty(
            this, "mondrian.server.IdleOrphanSessionTimeout", 3600);
    ...
    /**
     * <p>
     *                 If true, SmartMemberReader will load all hierarchy members to the cache on first call.
     *                 Default: false.
     *             </p>
     */
    public transient final BooleanProperty LoadAllMembersToCache =
        new BooleanProperty(
            this, "mondrian.server.LoadAllMembersToCache", false);
    ...
    /**
     * <p>Boolean property enabling opt-in alias matching for explicit aggregate
     * levels that map to the same physical star-key column.</p>
     *
     * <p>When enabled, explicit aggregate matching may treat two levels as
     * equivalent only if they have different unique names but resolve to the same
     * underlying star column or key expression. This is intended for carefully
     * controlled flat-versus-hierarchical schema aliases and remains disabled by
     * default to avoid unsafe aggregate hits.</p>
     */
    public transient final BooleanProperty MatchAliasLevelsByStarColumn =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.MatchAliasLevelsByStarColumn", false);
    ...
    /**
     * <p>
     *                 Sets a memory threshold (in Kb).
     *                 An OutOfMemory exception is thrown when this threshold is reached.
     *                 If the value is 0, memory monitoring is disabled.
     *                 Default value: 0.
     *             </p>
     */
    public transient final IntegerProperty MemoryThreshold =
        new IntegerProperty(
     ...
     * annotations are evaluated via a user-provided SQL template instead
     * of per-tuple MDX evaluation. The SQL template, variables, and axis
     * limits are defined in schema XML annotations on the calculated
     * member.</p>
     */
    public transient final BooleanProperty NativeSqlEnable =
        new BooleanProperty(
    ...
    /**
     * <p>Internal diagnostic escape hatch for a canary, not a supported
     * setting. Leave it enabled.</p>
     *
     * <p>When enabled, NonEmptyCrossJoin judges every crossing it is about
     * to return by evaluating it at its own coordinate, and keeps only the
     * crossings a displayed fact-reading measure (else the context measure)
     * finds non-empty.</p>
     *
     * <p>Setting it to false restores the earlier, known-incorrect
     * NonEmptyCrossJoin behaviour: the candidates are returned as found, so
     * crossings whose value is empty can appear in the result. The test
     * suite assumes it is on; 14 tests fail by design with it off.</p>
     */
     ...
     * only those &lt;code&gt;&amp;lt;Property&amp;gt;&lt;/code&gt; columns that the
     * current query statically requires: literal
     * &lt;code&gt;.Properties("X")&lt;/code&gt; references in axes / slicer / WITH
     * MEMBER, plus &lt;code&gt;DIMENSION PROPERTIES&lt;/code&gt; per axis. Other
     * properties on touched levels are skipped from tuple/member reader
     * SQL projection for this query.&lt;/p&gt;
     *
     * &lt;p&gt;Fail-safe to eager (V2 leaves the level alone, V1-narrow / pre-V2
     * behaviour applies):&lt;/p&gt;
     * &lt;ul&gt;
     * &lt;li&gt;level not mentioned by any literal reference / DIM PROPS for this
     * query;&lt;/li&gt;
     * &lt;li&gt;MDX uses &lt;code&gt;.Properties(Iif(...))&lt;/code&gt;, computed name,
     * &lt;code&gt;StrToMember&lt;/code&gt;, &lt;code&gt;StrToTuple&lt;/code&gt; or
     * &lt;code&gt;StrToSet&lt;/code&gt; anywhere → affected hierarchy (or whole query
     * for &lt;code&gt;StrTo*&lt;/code&gt;) → eager;&lt;/li&gt;
     * &lt;li&gt;any DIMENSION PROPERTIES id cannot be resolved to a hierarchy →
     * whole query → eager (cannot tell which level was meant);&lt;/li&gt;
     * &lt;li&gt;level has a &lt;code&gt;MemberFormatter&lt;/code&gt; or any of its
     * properties carries a &lt;code&gt;PropertyFormatter&lt;/code&gt; — V2 cannot
     * statically analyse Java formatter code, so the level is eager.&lt;/li&gt;
     * &lt;/ul&gt;
     *
     * &lt;p&gt;Limitations of the current implementation (tracked in
     * dronsv/mondrian#22):&lt;/p&gt;
     * &lt;ul&gt;
     * &lt;li&gt;Schema-defined &lt;code&gt;&amp;lt;CalculatedMember&amp;gt;&lt;/code&gt;,
     * &lt;code&gt;&amp;lt;NamedSet&amp;gt;&lt;/code&gt; and &lt;code&gt;&amp;lt;Role&amp;gt;&lt;/code&gt;
     * expressions are NOT walked by the visitor; only the per-query AST is.
     * If a schema-side calculated member references properties not also
     * referenced by the user MDX, V2 may prune them.&lt;/li&gt;
     * &lt;li&gt;No lazy fetch on
     * &lt;code&gt;member.getPropertyValue(skipped)&lt;/code&gt; — returns null
     * silently (V2-M4 pending).&lt;/li&gt;
     * &lt;/ul&gt;
     *
     ...
     * (schema version, role/session/security scope) can reuse previously computed
     * result without re-running SQL. Works for XMLA clients (Excel) and olap4j/API
     * clients that run against this Mondrian instance.</p>
     */
    public transient final BooleanProperty ResultCacheEnabled =
        new BooleanProperty(
            this, "mondrian.rolap.resultCache.enabled", false);

    /**
     * <p>Upper bound of result cell count eligible for server-side exact-query
     * cache. Set to 0 or negative value to disable this size guard.</p>
     */
    public transient final IntegerProperty ResultCacheMaxCellCount =
        new IntegerProperty(
            this, "mondrian.rolap.resultCache.maxCellCount", 300000);

    /**
     * <p>Maximum number of exact-query results kept in server-side result cache.
     * Least-recently-used entries are evicted first.</p>
     */
    public transient final IntegerProperty ResultCacheMaxEntries =
        new IntegerProperty(
            this, "mondrian.rolap.resultCache.maxEntries", 32);

    /**
     * <p>Server-side result cache time-to-live in milliseconds.</p>
     */
    public transient final IntegerProperty ResultCacheTtlMillis =
        new IntegerProperty(
            this, "mondrian.rolap.resultCache.ttlMillis", 120000);
    ...
    /**
     * <p>Boolean property enabling conservative distinct-branch split by aggregate
     * table candidate for mixed measure batches.</p>
     *
     * <p>When enabled and mixed distinct splitting is active, Mondrian may execute
     * distinct-count measures in several branches grouped by their preferred
     * aggregate table candidate. This reduces the chance of a single distinct branch
     * falling back to fact when distinct measures are best served by different agg
     * tables.</p>
     */
    public transient final BooleanProperty SplitDistinctMeasuresByAggCandidate =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.SplitDistinctMeasuresByAggCandidate", true);

    /**
     * <p>Boolean property enabling conservative additive-branch split by aggregate
     * table candidate for mixed measure batches.</p>
     *
     * <p>When enabled and mixed measure splitting is active, Mondrian may execute
     * additive measures in several branches grouped by their preferred aggregate
     * table candidate. This reduces the chance of a single mixed additive branch
     * falling back to fact when measures are best served by different agg tables.</p>
     */
    public transient final BooleanProperty SplitMeasuresByAggCandidate =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.SplitMeasuresByAggCandidate", true);

    /**
     * <p>Maximum number of additive branches created by aggregate-candidate split
     * within one mixed batch.</p>
     *
     * <p>If candidate diversity exceeds this threshold, Mondrian skips this split
     * and executes additive measures as a single branch.</p>
     */
    public transient final IntegerProperty SplitMeasuresByAggCandidateMaxBranches =
        new IntegerProperty(
            this, "mondrian.rolap.aggregates.SplitMeasuresByAggCandidateMaxBranches", 4);

    /**
     * <p>Boolean property that forces split execution for mixed measure sets
     * containing both additive measures (for example SUM) and distinct-count
     * measures.</p>
     *
     * <p>When enabled, Mondrian runs distinct and non-distinct measures in
     * separate aggregation loads. This can improve aggregate table selection
     * when one SQL cannot match both measure classes efficiently.</p>
     *
     * <p>If not explicitly set, Mondrian may auto-enable split when
     * <code>DistinctCountMergeFunction</code> is configured. This keeps mixed
     * measure planning aligned with aggregate-state distinct-count setups while
     * preserving explicit <code>true/false</code> overrides.</p>
     *
     * <p>Default property value is false; explicit false always disables split.</p>
     */
    public transient final BooleanProperty SplitMixedDistinctMeasureBatches =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.SplitMixedDistinctMeasureBatches", false);

    /**
     * <p>Boolean property enabling lightweight branch telemetry for mixed
     * measure split execution.</p>
     *
     * <p>When enabled, Mondrian logs limited per-query events indicating
     * distinct/additive branch composition for easier diagnostics.</p>
     */
    public transient final BooleanProperty SplitMixedDistinctTelemetry =
        new BooleanProperty(
            this, "mondrian.rolap.aggregates.SplitMixedDistinctTelemetry", false);

    /**
     * <p>Maximum number of split telemetry log events emitted per query when
     * <code>SplitMixedDistinctTelemetry</code> is enabled.</p>
     */
    public transient final IntegerProperty SplitMixedDistinctTelemetryMaxPerQuery =
        new IntegerProperty(
            this, "mondrian.rolap.aggregates.SplitMixedDistinctTelemetryMaxPerQuery", 2);
    ...
    /**
     * <p>Enables lightweight server-side cache for XMLA metadata rowsets used by
     * Excel browse and field-list flows.</p>
     *
     * <p>The cache is scoped by request type, security context and discover
     * restrictions/properties. It caches only allow-listed metadata rowsets and
     * skips large responses.</p>
     */
    public transient final BooleanProperty XmlaDiscoveryCacheEnabled =
        new BooleanProperty(
            this, "mondrian.xmla.discoveryCache.enabled", true);

    /**
     * <p>Maximum number of XMLA discovery rowsets kept in server-side metadata
     * cache. Least-recently-used entries are evicted first.</p>
     */
     ...
     * Large browse responses, such as high-cardinality member lists, bypass the
     * cache when they exceed this threshold.</p>
     */
